...
export async function findRelevantTag(prompt) {
  ...
  const { response } = await model.generateContent(
    `here is the prompt given by the user : ${prompt} and i want you to return only one relevant tag for this prompt from this list :
     ["web development","mechanical","cooking","coding","art","education","electronics","gaming","sports"]. 
     just return the output as a string of one word no other text or anything else.`
  );
